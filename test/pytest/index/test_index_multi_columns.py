...
def test_create_table():
    sql = "create table Student(id varchar(32) primary key, name varchar(48), age int, grade varchar(16), sex varchar(4), birth date, phone varchar(13), address varchar(100), createTime timestamp);\n"
    ret = client.execute(sql)
    assert ret["success"] == True

def test_insert_mock_data():
    with open('test/pytest/support/students_100000.json', 'r', encoding='utf-8') as f:
...
def test_index_hit():
    sql = "explain select * from Student where name = '胡刚' and phone='15691709944' and address='南京市福田区新华路957号';"
    ret = client.execute(sql)
    assert ret["success"] == True
...
def test_perf_my_index():
    ret1 = client.execute("select * from Student where id = '98989';")
    assert ret1["success"] == True
    assert ret1["rows"] == 1
    ret2 = client.execute("select * from Student where name = '胡刚' and phone='15691709944' and address='南京市福田区新华路957号';")
