...
static List *MetaTableGenerateDescribeResult(Oid tid, MetaTable *meta_table) {
    ...
    foreach (lc, meta_table->meta_columns) {
        ...
        append_list(
            child_list, 
            new_key_value("field", meta_column->column_name, T_VARCHAR, tid, meta_column->type_oid, meta_column->array_dim)
        );
    
        /* key */
        append_list(
            child_list, 
            new_key_value("key", GetKeyTypeName(meta_column), T_VARCHAR, tid, meta_column->type_oid, meta_column->array_dim)
        );
        ...
        append_list(
            child_list, 
            new_key_value("length", &column_length, T_INT, tid, meta_column->type_oid, meta_column->array_dim)
        );
        ...
        append_list(
            child_list, 
            new_key_value("array", &is_array, T_BOOL, tid, meta_column->type_oid, meta_column->array_dim)
        );
        ...
        if (is_array)  {
            append_list(
                child_list, 
                new_key_value("array_dim", &meta_column->array_dim, T_BOOL, tid, meta_column->type_oid, meta_column->array_dim)
            );
        }
        ...
        switch (meta_column->default_value_type) {
            ...
            case DEFAULT_VALUE_NULL:
                append_list(
                    child_list, 
                    new_key_value("default", NULL, meta_column->column_type, tid, meta_column->type_oid, meta_column->array_dim)
                );
                break;
            case DEFAULT_VALUE:
                append_list(
                    child_list, 
                    new_key_value("default", meta_column->default_value, meta_column->column_type, tid, meta_column->type_oid, meta_column->array_dim)
                );
                break;
        ...
        if (meta_column->has_comment) {
            append_list(
                child_list, 
                new_key_value("comment", meta_column->comment, T_VARCHAR, tid, meta_column->type_oid, meta_column->array_dim)
            );
        }
