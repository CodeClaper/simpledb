...
static void *get_leaf_node_body(void *node, uint32_t default_value_len) {
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        return (node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
...
/* Get leaf node cell. */
void *get_leaf_node_cell(void *node, uint32_t key_len, uint32_t value_len, uint32_t default_value_len, uint32_t index) {
...
void set_leaf_node_cell_key(void *node, uint32_t index, uint32_t key_len, uint32_t value_len, uint32_t default_value_len, void *key) {
    uint32_t cell_len = key_len + value_len;
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        memcpy(node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
...
/* Get leaf node  refer value. */
Refer *get_leaf_node_index_refer_value(void *node, uint32_t key_len, uint32_t value_len, uint32_t default_value_len, uint32_t index) {
    return (Refer *) get_leaf_node_cell(node, key_len, value_len, default_value_len, index);
}
...
void set_internal_node_right_child(void *node, uint32_t default_value_len, uint32_t right_child_page_num) {
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        *(uint32_t *)(node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
                ROOT_NODE_META_COLUMN_SIZE * column_size + default_value_len + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE) = right_child_page_num;
    } else {
...
/* Get right child key. */
void *get_internal_node_right_child_key(void *node, uint32_t default_value_len) {
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        return (node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
                ROOT_NODE_META_COLUMN_SIZE * column_size + default_value_len + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE);
    } else {
        return (node + RIGHT_CHILD_OFFSET  + RIGHT_CHILD_SIZE);
    }
}
...
void set_internal_node_right_child_key(void *node, uint32_t key_len, uint32_t default_value_len, void *key) {
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        memcpy(node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
                ROOT_NODE_META_COLUMN_SIZE * column_size + default_value_len + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE, key, key_len);
    } else {
        memcpy(node + RIGHT_CHILD_OFFSET + RIGHT_CHILD_SIZE, key, key_len);
    }
}

/* Get internal node body pointer. */
static void *get_internal_node_body(void *node, uint32_t default_value_len) {
    if (is_root_node(node)) {
        uint32_t column_size = get_column_size(node);
        return (node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
                ROOT_NODE_META_COLUMN_SIZE * column_size + default_value_len); 
...
void *get_internal_node_key(void *node, uint32_t index, uint32_t key_len, uint32_t default_value_len) {
    ...
    if (is_root_node(node)) {
        ...
        return (node + internal_head_len + cell_len * index + INTERNAL_NODE_CELL_CHILD_SIZE);
    }
}
...
void set_internal_node_key(void *node, uint32_t index, void *key, uint32_t key_len, uint32_t default_value_len) {
    ...
    if (is_root_node(node)) {
        ...
        uint32_t internal_head_len = COMMON_NODE_HEADER_SIZE + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + key_len;
        memcpy(node + internal_head_len + cell_len * index + INTERNAL_NODE_CELL_CHILD_SIZE, key, key_len);
    }
}
...
uint32_t get_internal_node_child(void *node, uint32_t index, uint32_t key_len, uint32_t default_value_len) {
    ...
    if (is_root_node(node)) {
        ...
        uint32_t internal_head_len = COMMON_NODE_HEADER_SIZE + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + key_len;
        return *(uint32_t *)(node + internal_head_len + cell_len * index);
    }
}
...
void set_internal_node_child(void *node, uint32_t index, uint32_t page_num, uint32_t key_len, uint32_t default_value_len) {
    ...
    if (is_root_node(node)) {
        ...
        *(uint32_t *)(node + ROOT_NODE_META_COLUMN_SIZE_OFFSET + ROOT_NODE_META_COLUMN_SIZE_SIZE + 
        ...
        uint32_t internal_head_len = COMMON_NODE_HEADER_SIZE + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + key_len;
        *(uint32_t *)(node + internal_head_len + cell_len * index) = page_num;
    }
}
...
void *get_internal_node_cell(void *node, uint32_t index, uint32_t key_len, uint32_t default_value_len) {
    ...
    if (is_root_node(node)) {
        ...
        uint32_t internal_head_len = COMMON_NODE_HEADER_SIZE + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + key_len;
        return (node + internal_head_len + cell_len * index);
    }
}
 ...
 * */
static void *get_max_key(Table *table, void *node, uint32_t key_len, 
                         uint32_t value_len, uint32_t default_value_len) {
    switch (get_node_type(node)) {
        case INTERNAL_NODE: {
            return get_internal_node_right_child_key(node, default_value_len);
        }
        case LEAF_NODE: {
            uint32_t cell_num = get_leaf_node_cell_num(node, default_value_len);
            return get_leaf_node_cell_key(node, cell_num - 1, key_len, value_len, default_value_len);
        }
        default:
...
static void update_internal_node_key(Table *table, uint32_t page_num, void *old_key, void *new_key, 
    ...
    if (!GT(GetComparableValue(old_key, key_data_type), 
        ...
        UpgradeLockBuffer(buffer);
        set_internal_node_right_child_key(internal_node, key_len, default_value_len, new_key);
        DowngradeLockBuffer(buffer);
        MakeBufferDirty(buffer);
    }
...
void resort_internal_node_cells(Table *table, uint32_t page_num, uint32_t key_len, 
    ...
    {
        void *right_key = get_max_key(table, internal_node, key_len, value_len, default_value_len);
        uint32_t right_value = get_internal_node_right_child(internal_node, default_value_len);
        InternalNodeCellEntry *entry = instance(InternalNodeCellEntry);
        entry->key = right_key;
    ...
    for (index = 0; index < *keys_num; index++) {
        ...
        set_internal_node_child(internal_node, index, entry->value, key_len, default_value_len);
    }
    ...
    /* Reset into internal node right cell. */
    {
        ...
        InternalNodeCellEntry *entry = (InternalNodeCellEntry *) lfirst(cell);
        set_internal_node_right_child(internal_node, default_value_len, entry->value);
        set_internal_node_right_child_key(internal_node, key_len, default_value_len, entry->key);
    }
...
static void copy_root_to_leaf_node(Table *table, uint32_t new_page_num, 
    ...
    for (i = 0; i < *cell_num; i++) {
        set_leaf_node_cell_key(
            ...
            get_leaf_node_cell_key(root, i, key_len, value_len, default_value_len)
        ); 
        memcpy(
            get_leaf_node_cell_value(leaf_node, key_len, value_len, default_value_len, i), 
            get_leaf_node_cell_value(root, key_len, value_len, default_value_len, i), 
            value_len
        );
        /* Update refer. */
...
static void create_new_root_node(Table *table, uint32_t right_child_page_num, 
    ...
    switch(node_type) {
        case LEAF_NODE:
            copy_root_to_leaf_node(table, next_unused_page_num, key_len, value_len, default_value_len);
            break;
        case INTERNAL_NODE:
    ...
    set_internal_node_keys_num(root, default_value_len, 1);

    /* Register the new (leaf) leaf node to root. */
    void *left_max_key = get_max_key(table, left_child, key_len, value_len, default_value_len);
    set_internal_node_key(root, 0, left_max_key, key_len, default_value_len);
    set_internal_node_child(root, 0, next_unused_page_num, key_len, default_value_len);
    void *right_max_key = get_max_key(table, right_child, key_len, value_len, default_value_len);
    set_internal_node_right_child(root, default_value_len, right_child_page_num);
    set_internal_node_right_child_key(root, key_len, default_value_len, right_max_key);
...
/* Insert internal node a new cell. */
static void insert_internal_node_new_cell(Table *table, uint32_t page_num, uint32_t new_child_page_num, 
                                          uint32_t key_len, uint32_t value_len, uint32_t default_value_len) {
    Oid oid;
    Buffer buffer, new_child_buffer;
    void *internal_node, *new_child_node, *new_child_max_key;
    MetaColumn *primary_key_meta_column;
    uint32_t *keys_num, right_child_page_num;
    void *right_child_max_key;
    ...
    if (GE(GetComparableValue(new_child_max_key, primary_key_meta_column->column_type), 
        ...
        set_internal_node_right_child(internal_node, default_value_len, new_child_page_num);
        set_internal_node_right_child_key(internal_node, key_len, default_value_len, new_child_max_key);
    } else {
        ...
        if (EQ(GetComparableValue(default_key, primary_key_meta_column->column_type), 
            ...
            set_internal_node_key(internal_node, new_child_max_key_index + 1, new_child_max_key, key_len, default_value_len);
            set_internal_node_child(internal_node, new_child_max_key_index + 1, new_child_page_num, key_len, default_value_len);
        } else {
            /* Move the right cells and make space for the new one. */
            int i;
            ...
            set_internal_node_key(internal_node, new_child_max_key_index, new_child_max_key, key_len, default_value_len);
            set_internal_node_child(internal_node,  new_child_max_key_index, new_child_page_num, key_len, default_value_len);
        }
    }
...
static void insert_and_split_internal_node(Table *table, uint32_t old_internal_page_num, uint32_t new_child_page_num, 
    ...
    if (GT(GetComparableValue(new_child_max_key, primary_key_meta_column->column_type), 
        ...
        set_internal_node_right_child_key(old_internal_node, key_len, default_value_len, new_child_max_key);
        new_child_max_key = right_child_max_key;
    ...
    void *old_max_key = copy_value2(get_max_key(table, old_internal_node, key_len, value_len, default_value_len), primary_key_meta_column);
    ...
    for (i = keys_num; i >= 0; i--) {
        ...
        if (i == new_child_max_key_index) {
            set_internal_node_key(destination_node, index, new_child_max_key, key_len, default_value_len); 
            set_internal_node_child(destination_node, index, new_child_page_num, key_len, default_value_len);
        } else if (i > new_child_max_key_index) {
            /* Right cells make cell space. */
            memcpy(get_internal_node_cell(destination_node, index, key_len, default_value_len), 
                   get_internal_node_cell(old_internal_node, i - 1, key_len, default_value_len), 
                   cell_len);
        } else {
            memcpy(get_internal_node_cell(destination_node, index, key_len, default_value_len), 
                   get_internal_node_cell(old_internal_node, i, key_len, default_value_len), 
                   cell_len);
        }
    }
    ...
    set_internal_node_right_child_key(
        new_internal_node,
        key_len,
        default_value_len,
        get_internal_node_right_child_key(old_internal_node, default_value_len)
    );
    ...
    set_internal_node_right_child_key(
        ...
        key_len,
        default_value_len,
        get_internal_node_key(old_internal_node, LEFT_SPLIT_COUNT - 1, key_len, default_value_len)
    );
...
static void insert_internal_node_cell(Table *table, uint32_t page_num, uint32_t new_child_page_num, 
    ...
    if (overflow_internal_node(internal_node, *keys_num, key_len, default_value_len))
        insert_and_split_internal_node(table, page_num, new_child_page_num, key_len, value_len, default_value_len);
    else 
        insert_internal_node_new_cell(table, page_num, new_child_page_num, key_len, value_len, default_value_len);
...
static void insert_leaf_node_new_cell(Row *row, Refer *refer, uint32_t key_len, 
    ...
    destination = seriable_index_value(row, refer);
    ...
    if (!is_root_node(node) && refer->cell_num == *cell_num) {
        ...
        parent_page_num = get_parent_pointer(node);
        old_max_key = get_leaf_node_cell_key(node, *cell_num - 1, key_len, value_len, default_value_len);
        primary_key_meta_column = MetaTableFindPrimaryKey(table->meta_table);
        /* Logic check.*/
        ...
        update_internal_node_key(table, parent_page_num, 
                                 old_max_key, key, 
                                 key_len, value_len, default_value_len, 
                                 primary_key_meta_column->column_type);
    }
...
static void insert_and_split_leaf_node(Row *row, Refer *refer, uint32_t key_len, 
    ...
    void *old_max_key = copy_value2(get_max_key(table, old_node, key_len, value_len, default_value_len), primary_key_meta_column);
    ...
    for (i = *cell_num; i >= 0; i--) {
        ...
        void *destination = get_leaf_node_cell(destination_node, key_len, value_len, default_value_len, index_at_node);
        ...
        if (i == refer->cell_num) {
            /* Deposit cursor. */
            void *serial_data = seriable_index_value(row, refer);
    ...
    if (is_root_node(old_node)) 
         ...
         * We'll resort parent internal node cells lately.
         * */
        void *new_max_key = get_max_key(table, old_node, key_len, value_len, default_value_len);
        update_internal_node_key(
            table, parent_page_num, 
        ...
        /* Parent internla node cells may mass, so resort the cells. */
        resort_internal_node_cells(
            table, parent_page_num, key_len, value_len, 
            default_value_len, primary_key_meta_column->column_type
        );
...
static bool overflow_root_internal_node_new_column(void *root_node, MetaColumn *new_column, uint32_t key_len, uint32_t default_value_len) {
    uint32_t cell_len = key_len + INTERNAL_NODE_CELL_CHILD_SIZE;
    ...
    uint32_t after_len = COMMON_NODE_HEADER_SIZE + ROOT_NODE_META_COLUMN_SIZE_SIZE + ROOT_NODE_META_COLUMN_SIZE * (column_size + 1) + 
                         default_value_len + KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + cell_len * keys_num;
    return after_len > PAGE_SIZE;
}
...
static void split_root_leaf_node_append_column(uint32_t page_num, Table *table, MetaColumn *new_column, uint32_t pos) {
    ...
    new_buffer = ReadBuffer(oid, next_unused_page_num);
    new_node = GetBufferPage(new_buffer);
    initial_leaf_node(new_node, default_value_len, false);
...
static void split_root_internal_node_append_column(uint32_t page_num, Table *table) {
    ...
    for (i = keys_num - 1; i >= 0; i--) {
        ...
        /* Copy memory. */
        memcpy(
            get_internal_node_cell(destination_node, index, key_len, default_value_len), 
            get_internal_node_cell(old_internal_node, i, key_len, default_value_len), 
            cell_len
        );
    }
    ...
    set_internal_node_right_child(new_internal_node, 
                                  default_value_len, 
                                  get_internal_node_right_child(old_internal_node, default_value_len));
    set_internal_node_right_child_key(new_internal_node,
                                      key_len, 
                                      default_value_len, 
                                      get_internal_node_right_child_key(old_internal_node, default_value_len));
    ...
    set_internal_node_right_child(old_internal_node, 
                                  default_value_len, 
                                  get_internal_node_child(old_internal_node, LEFT_SPLIT_COUNT - 1, key_len, default_value_len));
    set_internal_node_right_child_key(old_internal_node, 
                                      key_len,
                                      default_value_len,
                                      get_internal_node_key(old_internal_node, LEFT_SPLIT_COUNT - 1, key_len, default_value_len));
    set_internal_node_keys_num(old_internal_node, default_value_len, LEFT_SPLIT_COUNT - 1);
...
static void append_leaf_node_column(uint32_t page_num, Table *table, MetaColumn *new_column, int pos) {
    ...
    if (is_root_node(leaf_node)) {
        if (overflow_leaf_node_new_column(leaf_node, new_column, key_len, value_len, default_value_len)) {
            split_root_leaf_node_append_column(page_num, table, new_column, pos);
...
static void append_root_internal_node_column(uint32_t page_num, Table *table, MetaColumn *new_column, int pos) {
    ...
    if (overflow_root_internal_node_new_column(root_node, new_column, key_len, default_value_len)) {
        ...
        memmove(
            ...
            body,
            KEYS_NUM_SIZE + INTERNAL_NODE_NEXT_SIBLING_SIZE + RIGHT_CHILD_SIZE + cell_len * keys_num
        );
...
void delete_internal_node_cell(Table *table, uint32_t page_num, void *key, DataType key_data_type) {
    ...
    Buffer buffer;
    void *internal_node; 
    uint32_t key_num, key_len, cell_len, value_len, default_value_len, key_index;
                ...
                key_data_type)) 
    {
        ...
        if (key_num == 1) 
            set_internal_node_right_child(internal_node, default_value_len, 0);
        else if (key_num == 0) {
            set_internal_node_right_child(internal_node, default_value_len, 0);
            if (is_root_node(internal_node))
            ...
            void *old_max_key = get_max_key(table, internal_node, key_len, value_len, default_value_len);
            void *new_max_key = get_internal_node_key(internal_node, key_num - 1, key_len, default_value_len);
            uint32_t max_key_page_num = get_internal_node_child(internal_node, key_num - 1, key_len, default_value_len);
            set_internal_node_right_child(internal_node, default_value_len, max_key_page_num);
            set_internal_node_right_child_key(internal_node, key_len, default_value_len, new_max_key);
            memset(get_internal_node_cell(internal_node, key_num - 1, key_len, default_value_len), 0, cell_len);
            /* Decrease cell number. */
            set_internal_node_keys_num(internal_node, default_value_len, --key_num);
            /* Update key.*/
            update_internal_node_key(
        ...
        if (key_index == key_num - 1) {
            ...
            /* Make last one NULL. */
            memset(get_internal_node_cell(internal_node, key_index, key_len, default_value_len), 0, cell_len);
            /* Decrease cell number. */
            set_internal_node_keys_num(internal_node, default_value_len, --key_num);
        } else {
...
void delete_row_data(void *key, Refer *refer) {
    ...
    uint32_t key_len, value_len, default_value_len, cell_length, cell_num;
    MetaColumn *primary_key_meta_column;
    ...
    if (refer->cell_num == cell_num - 1) {
        ...
        if (!is_root_node(leaf_node)) {
            ...
            if (refer->cell_num == 0)
                delete_internal_node_cell(table, parent_page, obs_key, primary_key_meta_column->column_type);
            else {
                void *obs_previous_key = get_leaf_node_cell_key(leaf_node, refer->cell_num - 1, key_len, value_len, default_value_len);
                update_internal_node_key(
                    table, parent_page, obs_key, 
                    obs_previous_key, key_len, value_len, default_value_len,
                    primary_key_meta_column->column_type
                );
            }
        }
        ...
        memset(
            get_leaf_node_cell(leaf_node, key_len, value_len, default_value_len, cell_num - 1), 
            0, cell_length
        );
        ...
        for (uint32_t i = refer->cell_num; i < cell_num; i++) {
            if (i == cell_num - 1) 
                memset(get_leaf_node_cell(leaf_node, key_len, value_len, default_value_len, i), 0, cell_length);
            else {
                memcpy(get_leaf_node_cell(leaf_node, key_len, value_len, default_value_len, i), 
                       get_leaf_node_cell(leaf_node, key_len, value_len, default_value_len, i + 1), 
                       cell_length);
    ...
    /* Decrease cell number. */
    cell_num--;
    decrease_leaf_node_cell_num(leaf_node, default_value_len);
...
void update_index_refer_content(Table *table, Refer *iRefer, Refer *newRefer) {
    ...
    memcpy(destination, newRefer, REFER_SIZE);
